...
kb_File = './export/选课结果.html'
yzm_File = './export/yzm.jpg'
cjb_File = './export/成绩总表.html'
bjg_File = './export/不及格成绩总表.html'
css_File = './export/project.css'
        ...
        if(response.status_code == 200):
            ...
            if int(len(response.text)) > 2600:
                if '<h1>500' not in response.text:
                    kyl(response.text)
                    try:
                        response = POST(url_selected + xk,
                                        cookie, debug, xkqr_params)
                        print(response.status_code)
                        if(response.status_code == 200):
                            # with open('./xkjg.html', 'w') as f:
                            # 	f.write(response.text)
                            if '<h1>500' not in response.text:
                                print(xkjg(response.text), end=' ')
                            else:
            ...
            while vpncl:
                ...
                if(login(cookie, debug_mode)):
                    ...
                    while(choose):
                        print('===================================')
                        choose = int(
                            input('%s\n\n1、选课  2、退课  3、查课表  4、成绩总表\n5、抢课模式【非公开项目】  6、名字校验（刷新cookie有效期）\n0、退出当前教务系统账号（退出后可更换端口）\n\n请输入选项：' % usrname))
